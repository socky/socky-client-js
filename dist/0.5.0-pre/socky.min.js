...
Socky.Utils.extend(a,this.generate_subscription_payload());return a},authorize_via_ajax:function(a,b){var c=window.XMLHttpRequest?new XMLHttpRequest:new ActiveXObject("Microsoft.XMLHTTP"),d=this.generate_auth_payload();c.open("POST",this._socky.auth_endpoint(),true);c.setRequestHeader("Content-type","application/x-www-form-urlencoded");c.onreadystatechange=function(){if(c.readyState==4)if(c.status==200){var e=Socky.Utils.parseJSON(c.responseText);a(e)}else b();else b()};c.send("payload="+JSON.stringify(d))},
