...
null;this.on_socket_message({data:{event:"socky:connection:error",reason:"down"}})}},this),2E3)}else{this.log("WebSocket unavailable");this._connection={}}},on_socket_open:function(){this.log("connected to socket, waiting for connection_id");this._connection_open=true},on_socket_message:function(a){a=Socky.Utils.parseJSON(a.data);if(typeof a.data=="string")a.data=Socky.Utils.parseJSON(a.data);this.log("received message",a);this._trigger("raw",a.event,a);this._trigger("public",a.event,a);if(a.channel){var b=
